...
class TestPassHour:
    ...
    def test_that_idle_task_moves_forward_for_each_team_member(
        self, team, jessie, jim, john, jane
    ):
        team.pass_hour()

        assert team.idle_time() == jessie.skill + jim.skill + john.skill + jane.skill

    def test_that_idle_task_moves_forward_twice_for_each_team_member(
        self, team, jessie, jim, john, jane
    ):
        team.pass_hour()
        team.pass_hour()

        assert team.idle_time() == 2 * (
            jessie.skill + jim.skill + john.skill + jane.skill
        )

    def test_that_shared_tasks_moves_forward_for_each_person_working(
        self, team, jessie, jim, john, jane
    ):
